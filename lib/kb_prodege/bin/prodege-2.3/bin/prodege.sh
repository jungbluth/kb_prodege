...
if [[ -z $WORKING_DIR || -z $JOB_NAME ]]; then
...
byte_lim=200000
if [ "$size_bytes" -lt "$byte_lim" ]; then
...
if [ ! -e "$DB_EUK_LOCATION" ]; then
  DB_EUK_LOCATION=${INSTALL_LOCATION}/NCBI-nt-euk/nt_euks
fi

if [ "${RUN_GENECALL}" == "1" ]; then
	if [ -z "$PRODIGAL_EXE" ]; then
		PCmd=`which prodigal`
	else
		PCmd=$PRODIGAL_EXE
	fi
...
if [ "$RUN_BLAST" == "1" ]; then
	if [ -e ${INT_DIR}/${JOB_NAME}_genes.fna ]; then
		if [ -z "$BLASTN_EXE" ]; then
...
if [ "$RUN_CLASSIFY" == "1" ]; then
	if [ -e ${INT_DIR}/${JOB_NAME}_contigs.bins ]; then
		if [ -z "$R_EXE" ]; then
			RCmd=`which R`
		else
			RCmd=$R_EXE
		fi
    if [ -z "$KMER_CUTOFF" ]; then
      KMER_CUTOFF=DEFAULT
    fi
    ...
    echo `date` "### Begin contig LCA assignments" >> ${WORKING_DIR}/${JOB_NAME}_log
		...
		if [[ -e ${INT_DIR}/${JOB_NAME}_kmer_contam_contigs ]]; then
		...
		if [[ -e ${INT_DIR}/${JOB_NAME}_prodege_classify.out && ! -e ${INT_DIR}/${JOB_NAME}_kmer_contam_contigs ]]; then
